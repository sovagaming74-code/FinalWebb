...
// --- THE "NO GAPS" GOAL SCHEDULER LOGIC ---
...
goalForm.onsubmit = (e) => {
    ...
    const col = document.getElementById('goal-col-input').value;
    const row = document.getElementById('goal-row-input').value;

    // We add inline styles for height and margin to ensure there is NO gap
    const goalHtml = `
        <div class="slot smart-suggest" style="grid-column: ${col}; grid-row: ${row}; height: 100% !important; margin: 0 !important; width: 100% !important;">
            ⚡ ${name}
        </div>`;
    
    calendarGrid.insertAdjacentHTML('beforeend', goalHtml);
